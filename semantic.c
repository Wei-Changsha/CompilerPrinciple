...
char *newAlias() {
    ...
    char s[10];
    //itoa(no++,s,10);
    sprintf(s, "%d", no++); //将no++转为10进制表示的字符串。
    return strcat0("v",s);
}
    ...
    switch (T->kind){
        ...
        case ID:
            ...
            T->num=1;
                ...
                if (rtn==-1)
                     semantic_error(T->pos,T->type_id, "111变量重复定义");
                else T->place=rtn;
            T->num=1;
            }
            break;
        case ARRAY:{
            ...
            if(T->kind == ARRAY){  
                ...
                T->ptr[0]->type=T->type;              //将类型属性向下传递变量结点
                T->ptr[0]->offset=base + T->width*T->type_int;          //外部变量的偏移量向下传递
                T->ptr[0]->width= T->width*T->type_int;
                T->ptr[0]->isArray = 1;
               // printf("<%d> int intint\n",T->type_int);
                // T->ptr[1]->type=T->type;              //将类型属性向下传递变量结点
                // T->ptr[1]->offset=T->offset+T->width; //外部变量的偏移量向下传递
  ...
  if (T)
	...
	case ID:    //查符号表，获得符号表中的位置，类型送type
            if(T->isArray){   
                rtn=fillSymbolTable(T->type_id,newAlias(),LEV,T->type,'S',T->offset); 
                if (rtn==-1)
                semantic_error(T->pos,T->type_id, "fff数组变量未定义");
                else T->place=rtn;
                T->num=1;
            }
            else{
    ...
    case ARRAY: 
		        ...
		        // if(T->ptr[1]->type!=INT)semantic_error(T->pos,"", "数组变量的下标不是整型表达式"); 		
                // Exp(T->ptr[0]); 		
                // T->type=T->ptr[0]->type; 		
                ...
                if (rtn==-1)
                    semantic_error(T->ptr[0]->pos,T->ptr[0]->type_id, "数组变量未定义");
                if (symbolTable.symbols[rtn].flag=='F')
                    semantic_error(T->ptr[0]->pos,T->ptr[0]->type_id, "是函数名，类型不匹配");
           
                if(!T->ptr[0]->type!=INT){
                    semantic_error(T->ptr[0]->pos,T->ptr[0]->type_id, "数组变量的下标不是整型表达式");
                    break;
                }         //将类型属性向下传递变量结点
                //T->ptr[0]->offset=base + T->width*T->type_int;          //外部变量的偏移量向下传递
               // T->ptr[0]->width= T->width*T->type_int;
    ...
    case INT:   T->place=fill_Temp(newTemp(),LEV,T->type,'T',T->offset); //为整常量生成一个临时变量
                ...
                T->width=4;
                break;
    case CHAR:  T->place=fill_Temp(newTemp(),LEV,T->type,'T',T->offset); //为char常量生成一个临时变量
                T->type=CHAR;
	...
	case DMINUS:
             rtn=searchSymbolTable(T->ptr[0]->type_id);
            if (rtn==-1)
                semantic_error(T->ptr[0]->pos,T->ptr[0]->type_id, "变量未定义,不能对左值表达式进行自增、自减运算");
            if (symbolTable.symbols[rtn].flag=='F')
                semantic_error(T->ptr[0]->pos,T->ptr[0]->type_id, "是函数名，类型不匹配");
            if (T->ptr[0]->kind!=ID&&T->ptr[0]->kind!=ARRAY){
                semantic_error(T->ptr[0]->pos,"", "对非左值表达式进行自增、自减运算");
            }
            break;
	case PLUS:
	case MINUS:
...
{//对抽象语法树的先根遍历,按display的控制结构修改完成符号表管理和语义检查和TAC生成（语句部分）
  ...
  if (T)
	...
	case FUNC_DEF:      //填写函数定义信息到符号表
            ...
            T->ptr[2]->returnAddrType=!strcmp(T->ptr[0]->type_id,"int")?INT:FLOAT;//获取函数返回类型送到函数体结点

            strcpy(T->ptr[2]->Snext,newLabel());  //函数体语句执行结束后的位置属性
            semantic_Analysis(T->ptr[2]);         //处理函数体结点
            T->ifReturn=T->ptr[2]->ifReturn;
                      if(T->ifReturn!=1){
                semantic_error(T->pos,T->type_id, "函数没有返回语句");
            }
	...
	case COMP_STM:
            ...
            if (T->ptr[1]){
                ...
                T->ptr[1]->returnAddrType=T->returnAddrType;//将需要返回的类型向函数里面传递  传到子节点
                strcpy(T->ptr[1]->Snext,T->Snext);  //S.next属性向下传递
                semantic_Analysis(T->ptr[1]);       //处理复合语句的语句序列
                T->width+=T->ptr[1]->width;
                T->code=merge(2,T->code,T->ptr[1]->code);
                T->ifReturn= T->ptr[1]->ifReturn;
                }
             #if (DEBUG)
                ...
                while (T0) {  //处理所以DEC_LIST结点
                    ...
                    if (T0->ptr[0]->kind==ID){
                          if(T->isArray){
                            ...
                            rtn=fillSymbolTable(T->type_id,newAlias(),LEV,T->type,'S',T->offset);  //最后一个变量名
                            if (rtn==-1)
                            semantic_error(T->pos,T->type_id, "数组重复定义");//这里好像没有起作用
                            else T->place=rtn;
                            T->num=1;
                     }else{
                        ...
                        T->width+=width;
                          ...
                          T0->ptr[0]->ptr[0]->type=T0->ptr[0]->type;              //将类型属性向下传递变量结点
                          //T0->ptr[0]->ptr[0]->offset=base + T0->ptr[0]->width*T0->ptr[0]->type_int;          //外部变量的偏移量向下传递
                          //T->ptr[0]->width= T->width*T->type_int;
                          T0->ptr[0]->ptr[0]->isArray = 1;
	...
	case STM_LIST:
                ...
                T->ptr[0]->offset=T->offset;
                T->ptr[0]->loop= T->loop;
                T->ptr[0]->returnAddrType=T->returnAddrType;// 向下传递需要返回的类型
               
                semantic_Analysis(T->ptr[0]);
                T->ifReturn=T->ptr[0]->ifReturn;   //向上传递是否有返回类型
                T->code=T->ptr[0]->code;
                T->width=T->ptr[0]->width;
                if (T->ptr[1]){     //2条以上语句连接,S.next属性向下传递
                    ...
                    T->ptr[1]->loop= T->loop;     
                    T->ptr[1]->returnAddrType=T->returnAddrType;//2条以上语句连接 传递需要返回的类型
//                  T->ptr[1]->offset=T->offset+T->ptr[0]->width; //顺序结构顺序分配单元方式
                    semantic_Analysis(T->ptr[1]);
                    T->ifReturn=T->ptr[1]->ifReturn;   //向上传递是否有返回类型
...
//                        T->width+=T->ptr[1]->width;//顺序结构顺序分配单元方式
    ...
    case CONTINUE:
        //  result.kind=ID;   strcpy(result.id,T->type_id);
        //     result.offset=rtn;
        //     T->code=genIR(CONTINUE,opn1,opn2,result);     //生成中间代码：FUNCTION 函数名
        if(T->loop!=1){
            semantic_error(T->pos,T->type_id, "当前上下文不是循环");
        }
        T->code=merge(2,T->code,genGoto(T->Efalse));
        break;
    case EXP_STMT:
	...
	case RETURN:if (T->ptr[0]){
				 ...
				 /*需要判断返回值类型是否匹配*/
                    if(T->ptr[0]->type != T->returnAddrType){
                        semantic_error(T->pos,T->type_id, "函数返回值类型与函数定义的返回值类型不匹配");
                    }
